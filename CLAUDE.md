...
#### Publishing a reactive refresh — the bundle is only the FALLBACK

**The live Today's Reactive surface is driven by Supabase, NOT the bundled `FORMATION_DATA` block.** On load, `social.html`'s `applyPayload` (around l.3885) fetches the **newest `today_reactive` row for the market** (`?market=eq.Canada&order=created_at.desc&limit=1`) and **overrides** the bundled `FORMATION_DATA.ca`. The bundled block is the fallback shown only when no published row exists. So **editing `social.html` / `FORMATION_DATA.ca` and pushing `main` does NOT change what the team sees** while a published row is newer — you must publish a row too.

**Every reactive refresh = TWO steps:**
1. **Edit the bundle** (`social.html` → `FORMATION_DATA.ca`), syntax-check via JavaScriptCore, branch → merge → push `main`. This keeps the fallback + the source-of-record in sync.
2. **Publish a `today_reactive` row** so it goes live. Two ways:
   - **Preferred — MFG cockpit** (`mfg.html` → Reactive tab → Execute → **Publish**): also broadcasts on the `wcc_reactive` channel, so open reader tabs swap live with no reload.
   - **Direct REST insert** (when working from the CLI): build the payload by extracting `ca` from the bundle via JavaScriptCore (`payload = {formation, cultural, match, storyboards, assetElevations}`), then:
     ```
     POST {SB_URL}/rest/v1/today_reactive
       headers: apikey + Authorization: Bearer <publishable key>, Content-Type: application/json, Prefer: return=representation
       body: { "market": "Canada", "payload": <payload>, "raw_input": "<briefing summary>", "uploaded_by": "<name>", "live_date": "YYYY-MM-DD" }
     ```
     Anon INSERT is allowed (RLS is permissive). **Newest row wins** (`order=created_at.desc`); prior rows stay in history, so this is reversible by re-publishing. **Caveat:** a direct REST insert does NOT broadcast on `wcc_reactive`, so already-open reader tabs need a reload / visibility-flip to pick it up (the MFG Publish button does broadcast).
3. **Verify:** re-fetch the newest Canada row and confirm `formation.date`, spike titles, storyboards, and elevation lanes match today's content.

The `payload` carries `formation` + `cultural` + `match` + `storyboards` + `assetElevations` (same shapes as the bundle). The storyboard `isNewShape` guard requires `footage` + a `prompts[]` A/B/C array on every storyboard, or the whole payload is rejected as stale and the bundle shows instead.
